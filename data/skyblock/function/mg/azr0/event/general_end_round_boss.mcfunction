...
execute as @a[tag=MG_AZR0PT] at @s if score MG_AZR0_Timer rng2 matches 101 if score @s Azr0_Wave_best matches ..100 run tellraw @s [{"text": "解锁了新的游戏选项！","color": "gold",bold:true},{"text": "\n此刻起，开始新游戏时可以选择跳过前15波，直接进入第16波游戏。","color": "green",bold:false}]
