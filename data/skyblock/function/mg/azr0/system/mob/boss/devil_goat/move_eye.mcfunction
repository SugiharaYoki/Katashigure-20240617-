...
execute if score @s rng5 matches 2.. run particle soul ~ ~1 ~ 0.53 1 0.53 0.02 3
execute if score @s rng5 matches 2.. run particle crimson_spore ~ ~0.2 ~ 0.8 0.3 0.8 0 3
...
execute as @a[tag=MG_AZR0PT,tag=!azr0_player_looking_at_devil_goat] at @s if predicate {condition:"minecraft:entity_properties",entity:"this",predicate:{type_specific:{type:"minecraft:player",looking_at:{type:"minecraft:goat"}}}} run tag @s add azr0_player_looking_at_devil_goat_process
