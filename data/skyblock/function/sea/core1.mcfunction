...
execute if block 90074 103 136 lectern positioned 90128 136 20 if entity @a[tag=SEAPT,distance=0..20,limit=1] run summon minecraft:glow_item_frame ~ ~ ~ {Tags:[iframe_sea,iframe_sea_spectral],Invisible:0b,Fixed:0b,Invulnerable:0b,Facing:5b,Item:{id:"minecraft:silence_armor_trim_smithing_template",components:{custom_name:'{"text":"超频声荡 I","italic":false,"color":"red"}',custom_data:{sea_t_ripper1:true},lore:['{"text":"音爆 I 发射磁锯会制造超音波","color":"white","italic":false}','{"text":"可用于破坏绿色藤蔓","color":"white","italic":false}']}}}
...
execute if block 90102 124 125 iron_block positioned 90104 122 131 if entity @a[distance=..6.2,limit=1] run item replace block ~ ~ ~ container.16 with trial_key[custom_name='{"text":"工具维护室钥匙","italic":true,"color":"yellow","italic":false}',custom_data={"sea_key01":true}]
