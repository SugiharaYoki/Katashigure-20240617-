...
execute if entity @n[tag=SEA_follow_target_origin,distance=0..0.7] run tellraw @a[tag=SEA_follow_debug] {"text": "SEA_follow 再次开始追踪目标","color": "dark_gray"}
execute if entity @n[tag=SEA_follow_target_origin,distance=0..0.7] run kill @e[tag=SEA_follow_target_marker]
execute if entity @n[tag=SEA_follow_target_origin,distance=0..0.7] run kill @e[tag=SEA_follow_target_origin]
