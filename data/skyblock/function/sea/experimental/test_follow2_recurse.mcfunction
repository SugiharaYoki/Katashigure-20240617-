...
tellraw @a [{"text": "SEA_follow 进行递归 ","color": "dark_gray"},{"score":{"name":"@n[tag=SEA_follow_target_origin]","objective":"rng1"},"color":"dark_gray"}]
...
execute as @p[tag=SEA_follow_target] at @s as @n[tag=SEA_follow_target_marker,distance=..3] run tag @n[tag=SEA_follow_target_marker] add SEA_follow_target_destination
...
execute unless entity @e[tag=SEA_follow_target_destination] if entity @p[tag=SEA_follow_target,distance=..30] if entity @n[tag=SEA_follow_target_origin] run function skyblock:sea/experimental/test_follow2_recurse
