...
execute as @a[tag=SEAPT,x=90095,y=122,z=130,distance=0..1.4,tag=!e_i_07,tag=!SEAPF] at @s if score sea_player_count rng1 matches 2 run tellraw @a[tag=SEAPT,distance=0.1..] [{selector:"@s",color:"aqua"},{text:"：",color:"aqua",bold:1b},{text:"\n“文件室里好像还有留下什么，你见过钥匙吗？”",color:"white"}]
execute as @a[tag=SEAPT,x=90095,y=122,z=130,distance=0..1.4,tag=!e_i_07,tag=!SEAPF] at @s if score sea_player_count rng1 matches 3.. run tellraw @a[tag=SEAPT,distance=0.1..] [{selector:"@s",color:"aqua"},{text:"：",color:"aqua",bold:1b},{text:"\n“文件室里好像还有留下什么，你们谁见过钥匙？”",color:"white"}]
execute if entity @a[tag=SEAPT,x=90095,y=122,z=130,distance=0..1.4,tag=!e_i_07,tag=!SEAPF] run summon drowned 90097 123 148 {CustomName:{text:"安全帽腐尸"},DeathLootTable:"skyblock:sea_drowned1",Tags:["SEAmob"],attributes:[{id:"attack_damage",base:4.0},{id:"movement_speed",base:0.17}],equipment:{head:{id:iron_helmet,count:1}},drop_chances:{feet:0f,legs:0f,chest:0f,head:0f}}
...
execute store result score @n[tag=SEAtestfor1] Health run data get entity @n[tag=SEAtestfor1] Health
execute as @n[tag=SEAtestfor1] at @s run tp @s ~ ~ ~ facing 90142 123.0 117
execute as @n[tag=SEAtestfor1,scores={Health=50..99}] at @s run function skyblock:sea/e/ch1/ev001_mysterious_person
...
execute if entity @n[tag=sc,scores={sea_4temp2=3000..5000,sea_4temp5=8..34}] as @n[tag=SEAelectric] at @s run particle minecraft:flash{color:[1.000,1.000,1.000,1.00]} ~ ~-0.3 ~ 0.1 0.1 0.1 0.3 2
