...
execute if score sea_ch2_close_storage rng8 matches 21 if entity @a[scores={SEAPT_member=1}] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：\n“靠……我可不是电工。”","color":"white"}]
execute if score sea_ch2_close_storage rng8 matches 45 if entity @a[scores={SEAPT_member=1}] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：\n“不过至少冷冻库本身还是运作的，不用去费心思修电箱。”","color":"white"}]
execute if score sea_ch2_close_storage rng8 matches 85 if entity @a[scores={SEAPT_member=2}] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=2}]","color":"white"},{"text":"：\n“你说得轻巧，你带手电了吗？”","color":"white"}]
execute if score sea_ch2_close_storage rng8 matches 110 if entity @a[scores={SEAPT_member=2}] if entity @a[scores={SEAPT_member=3}] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=3}]","color":"white"},{"text":"：\n“带个屁手电噢，我现在只想要把步枪。”","color":"white"}]
execute if score sea_ch2_close_storage rng8 matches 150 if entity @a[scores={SEAPT_member=2}] if block 90132 130 136 redstone_lamp[lit=false] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：\n“反正就剩南边冷冻库了吧，怎么说？不要踩着我的脚。”","color":"white"}]
execute if score sea_ch2_close_storage rng8 matches 150 if entity @a[scores={SEAPT_member=2}] if block 90132 130 136 redstone_lamp[lit=true] run tellraw @a[tag=SEAPT] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：\n“那这就是第二处。这样北部冷冻库应该就可以通行了。”","color":"white"}]
