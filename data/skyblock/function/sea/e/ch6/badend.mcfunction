...
execute if score SEA_ch6_event rng8 matches 40 as @r[tag=SEAPT] at @s as @a[distance=0..200] run playsound minecraft:afterwordshort music @s ~ ~ ~ 1 1.0
...
execute if score SEA_ch6_event rng8 matches 215 as @a[tag=SEAPT] run tellraw @s [{text:"达成结局：",color:"gold",bold:1b},{text:"\nBad End「塞壬的谣曲」",color:"white",bold: false}]
