...
execute as @a[tag=SEAPT,x=90143,y=159,z=123,dx=3,dy=4,dz=3,tag=!e_i_53] at @s run playsound minecraft:garden2.chapter_title master @s ~ ~ ~ 1 1.3
...
execute as @a[tag=SEAPT,x=90143,y=159,z=123,dx=3,dy=4,dz=3,tag=!e_i_53] run tellraw @s [{text:"「WARNING: Point of No Return」",color:"red",bold:1b},{text:"\n这里是最后一处尚能回头的地点。\n继续前进后，你将不再有任何反悔的余地。\n确保你已经探索所有你想要探索的地方，并做好万全的战斗准备。\n在前方静候着你的是故事的最终篇章。",color:"red",bold: false}]
execute as @a[tag=SEAPT,x=90143,y=159,z=123,dx=3,dy=4,dz=3,tag=!e_i_53] run tellraw @s [{text:" - "},{text:"前行：并见证一切  ",color:"aqua"},{text:"〈◆〉",bold:1b,color:"aqua",click_event:{"action":"run_command","command":"trigger sea_crafter set 90008"}}]
