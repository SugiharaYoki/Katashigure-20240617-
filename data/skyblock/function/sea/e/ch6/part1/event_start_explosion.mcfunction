...
execute if score SEA_ch5_event_boss5 rng9 matches 310 as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200] at @s run tellraw @a[distance=0..200] [{"text":"菲尔娜：","color":"green","bold": true},{"text":"\n“主平台仓库上面的储油罐爆炸了，还好那里存的石油不多。”","color":"white","bold": false}]

execute if score SEA_ch5_event_boss5 rng9 matches 290 as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200] at @s as @p[tag=SEAPT] run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：什么动静？哪里爆炸了吗？","color":"white"}]
...
execute if score SEA_ch5_event_boss5 rng9 matches 333 as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200] at @s run tellraw @a[distance=0..200] [{"text":"菲尔娜：","color":"green","bold": true},{"text":"\n“不对……那里面存的东西不太对劲。离爆炸产生的烟尘远一点。”","color":"white","bold": false}]
