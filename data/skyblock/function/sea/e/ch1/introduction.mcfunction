...
execute if score sea_chapter_introduction sea_4temp1 matches 110 unless entity @a[scores={SEAPT_member=3}] if entity @a[scores={SEAPT_member=2}] run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：他们那通讯基站是豆腐渣工程吧，到底顶不顶用？","color":"white"}]
...
execute if score sea_chapter_introduction sea_4temp1 matches 180 unless entity @a[scores={SEAPT_member=3}] if entity @a[scores={SEAPT_member=2}] run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=2}]","color":"white"},{"text":"：哈哈，他们以为就我们俩能修一整座平台的通讯服务器不成。\n多半只是想先调查一下具体发生了什么吧。","color":"white"}]
