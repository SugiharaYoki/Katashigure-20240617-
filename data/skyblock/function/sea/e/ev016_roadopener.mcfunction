...
execute as @s[scores={sea_4temp1=130}] if block 90170 35 173 air run tellraw @a[tag=SEAPT] {"text": "陌生的女声：“不听劝阻的人可不是好孩子。”","color": "green"}

execute as @s[scores={sea_4temp1=67}] if block 90170 35 173 grindstone run tellraw @a[tag=SEAPT] {"text": "陌生的女声：“我已经给出了忠告。”","color": "green"}
execute as @s[scores={sea_4temp1=99}] if block 90170 35 173 grindstone run tellraw @a[tag=SEAPT] {"text": "陌生的女声：“但我看你好像并没有要放弃的样子。”","color": "green"}
execute as @s[scores={sea_4temp1=130}] if block 90170 35 173 grindstone run tellraw @a[tag=SEAPT] {"text": "陌生的女声：“这是最后一次警告，再往前我就会亲手杀了你。”","color": "green"}
