...
execute if score SEA_ch4_event_railbox rng1 matches 22 as @p[tag=SEAPT] at @s run tellraw @a[distance=0..250] [{selector:"@s",color:"aqua"},{text:"：",color:"aqua",bold:1b},{text:"\n“我这是把什么警报触发了……？”",color:"white"}]
execute if score SEA_ch4_event_railbox rng1 matches 92 as @p[tag=SEAPT] at @s run tellraw @a[distance=0..250] [{selector:"@s",color:"aqua"},{text:"：",color:"aqua",bold:1b},{text:"\n“好像……什么事都没发生。看来这里的警报系统已经没有意义了。”",color:"white"}]
