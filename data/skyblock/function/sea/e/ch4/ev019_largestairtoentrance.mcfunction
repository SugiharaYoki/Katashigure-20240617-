...
execute if entity @n[tag=SEAyuehan_ch4_acting] as @s[scores={sea_4temp1=30}] as @p[tag=SEAPT] at @s run tellraw @a[distance=0..250] [{text:"越涵：",color:"green",bold:1b},{text:"\n“等等……这群尸变体不太对劲。他们在闹内讧？”",color:"white",bold: false}]
...
execute if entity @n[tag=SEAyuehan_ch4_acting] as @s[scores={sea_4temp1=60}] as @p[tag=SEAPT] at @s run tellraw @a[distance=0..250] [{text:"越涵：",color:"green",bold:1b},{text:"\n“直到死亡……他们都还继续尝试守护忒尔克西。……唉，我这人没法去细想这类事情。”",color:"white",bold: false}]
