...
execute as @s[tag=SEAch3_spawn_timer7_pre,scores={sea_4temp1=1}] if entity @n[tag=sc,scores={sea_player=2..}] as @p[tag=SEAPT,distance=1..] run tellraw @a[distance=0..50] [{"selector":"@s","color":"white"},{"text":"：喂，你在干什么！不要往通讯平台走，快回来！！","color":"white"}]
