...
execute as @s[tag=sc,scores={sea_4temp2=171}] positioned 90133 137 108 as @p[tag=SEAPT] if entity @n[tag=sc,scores={sea_player=1..}] run tellraw @a[distance=0..50] [{selector:"@s",color:"aqua"},{text:"：",color:"aqua",bold: true},{text:"\n“通讯中心？是指在桥对面的附属平台吗？”",color:"white"}]
...
execute as @s[tag=sc,scores={sea_4temp2=227}] positioned 90133 137 108 run tellraw @a[distance=0..50,tag=SEAPT] [{text:" - "},{text:"询问：找到幸存者的希望有多大？  ",color:"yellow"},{text:"〈◆〉",bold:true,color:"gold",click_event:{"action":"run_command","command":"trigger sea_crafter set 90002"}}]
