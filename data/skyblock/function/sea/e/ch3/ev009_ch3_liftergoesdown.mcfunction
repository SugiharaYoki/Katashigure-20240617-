...
execute as @a[tag=SEAPT] at @s if block ~ ~-1 ~ barrier if entity @a[tag=SEAPT,distance=2..] if entity @n[tag=SEAyuehan,scores={sea_4temp5=..35}] unless entity @n[tag=SEAyuehan,tag=SEA_ch4elevator_end] unless entity @n[tag=SEA_ch4elevator_warn1] run tellraw @s [{text:"越涵：",color:"green",bold:1b},{text:"\n“人到齐了吗？最好一起下去，我不知道这电梯还能撑几次。”",color:"white",bold: false}]
