...
execute as @a[tag=SEAPT] at @s if block ~ ~-1 ~ barrier unless entity @a[tag=SEAPT,distance=2..] run tag @n[tag=SEAyuehan,x=90126,y=36,z=100,distance=10..,scores={sea_4temp5=36..}] add SEA_ch4elevator_start
    ...
    execute if entity @n[tag=SEAyuehan,tag=SEA_ch4elevator_start,scores={sea_4temp5=36..}] as @n[tag=SEAyuehan] at @s if entity @n[tag=sea_ch3elevator1,distance=0..4.1] run attribute @s generic.gravity base set 1.0
    #移动
