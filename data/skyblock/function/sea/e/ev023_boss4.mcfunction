...
execute as @s[scores={sea_4temp1=1}] as @n[tag=SEAboss4] at @s run playsound entity.villager.ambient hostile @a ~ ~ ~ 1 1.3
...
execute as @s[scores={sea_4temp1=52}] as @n[tag=SEAboss4] at @s run playsound entity.villager.ambient hostile @a ~ ~ ~ 1 1.3
execute as @s[scores={sea_4temp1=52}] run tellraw @a[tag=SEAPT] {"text": "陌生的女声：“他们没能破坏生态主控，但冷凝系统的过热使得研究设施无法再撑超过3小时。”","color": "green"}
...
execute as @s[scores={sea_4temp1=90..}] positioned 90184 56 48 unless entity @n[tag=SEAch4_spawn_timer_boss4_fast,distance=0..3] at @n[tag=SEAboss4] if entity @a[tag=SEAPT,distance=0..10] run summon marker ~ ~ ~ {Tags:["SEAch4_spawn_timer","SEAch4_spawn_timer_boss4_fast"]}
