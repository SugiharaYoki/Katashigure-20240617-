...
execute if score SEA_ch5_event_engineering_fiona sea_4temp2 matches 837.. as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200,tag=!SEAfiona_special_sentence_1] at @s if entity @n[tag=SEAmob,tag=!SEAnpc,distance=..4.5,type=!magma_cube] run tellraw @a[distance=0..200] [{"text":"菲尔娜：","color":"green","bold": true},{"text":"\n“哪来的这么多怪物，那群邪教徒也不清理一下场地吗？”","color":"white","bold": false}]
execute if score SEA_ch5_event_engineering_fiona sea_4temp2 matches 837.. as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200,tag=!SEAfiona_special_sentence_1] at @s if entity @n[tag=SEAmob,tag=!SEAnpc,distance=..4.5,type=!magma_cube] run tag @s add SEAfiona_special_sentence_1
