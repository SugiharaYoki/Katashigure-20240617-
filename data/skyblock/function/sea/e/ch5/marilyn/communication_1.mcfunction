...
execute if block 90163 36 86 green_concrete if score SEA_ch5_event_marilyn rng1 matches 67 positioned 90118 90 73 as @p[tag=SEAPT] if entity @n[tag=sc,scores={sea_player=1..}] run tellraw @a[distance=0..50] [{"selector":"@s","color":"white"},{"text":"：很奇怪，对吧？我和他认识明明才没几个小时。","color":"white"}]
execute unless block 90163 36 86 green_concrete if score SEA_ch5_event_marilyn rng1 matches 67 positioned 90118 90 73 as @p[tag=SEAPT] if entity @n[tag=sc,scores={sea_player=1..}] run tellraw @a[distance=0..50] [{"selector":"@s","color":"white"},{"text":"：能指引我下一步的路线吗？","color":"white"}]
