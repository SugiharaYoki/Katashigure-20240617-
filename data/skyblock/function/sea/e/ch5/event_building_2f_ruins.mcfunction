...
execute if entity @a[tag=SEAPT,tag=!SEAPF,x=90138,y=137,z=60,dx=6,dy=10,dz=9] if score SEA_ch5_event_building_fiona sea_4temp1 matches 43 if score SEA_ch5_event_fiona_favor rng1 matches 16.. run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：那可是热火朝天。但我们还是先听听他们的头子琼斯怎么说吧。","color":"white"}]
