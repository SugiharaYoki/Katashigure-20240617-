...
execute if score @s rng1 matches 224 at @s unless items entity @a[tag=SEAPT] container.* nether_star run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：恕我谢绝，琼斯先生。","color":"white"}]
execute if score @s rng1 matches 224 at @s if items entity @a[tag=SEAPT] container.* nether_star run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"white"},{"text":"：我不懂你在说什么。我们都是凡人，而且若是凡人能拥有神力，显然你也不可能如此崇拜天使。","color":"white"}]
...
execute if score @s rng1 matches 354..390 at @s positioned 90142 134 26 run scoreboard players set @a[tag=SEAPT,scores={sea_progress=..13}] sea_progress 14
