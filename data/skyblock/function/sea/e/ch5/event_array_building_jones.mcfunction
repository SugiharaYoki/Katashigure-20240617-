...
execute if score @s rng1 matches 81 at @s run tellraw @a[distance=0..250] [{"selector":"@r[tag=SEAPT]","color":"aqua"},{"text":"：\n“我算是都听明白了。你们这群教徒信仰的东西跟这些细菌无关。”","color":"white"}]
...
execute if score @s rng1 matches 131 at @s as @n[tag=SEAfiona,x=90000,y=100,z=0,distance=0..1200] at @s as @p[tag=SEAPT] run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"aqua"},{"text":"：\n“你说水晶镇压了那些细菌，靠的是什么？是你之前说过的大天使的神力吗？”","color":"white"}]
...
execute if score @s rng1 matches 224 at @s unless items entity @a[tag=SEAPT] container.* nether_star run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"aqua"},{"text":"：\n“恕我谢绝，琼斯先生。”","color":"white"}]
execute if score @s rng1 matches 224 at @s if items entity @a[tag=SEAPT] container.* nether_star run tellraw @a[distance=0..250] [{"selector":"@p[tag=SEAPT,scores={SEAPT_member=1}]","color":"aqua"},{"text":"：\n“我不懂你在说什么。我们都是凡人，而且若是凡人能拥有神力，显然你也不可能如此崇拜天使。”","color":"white"}]
