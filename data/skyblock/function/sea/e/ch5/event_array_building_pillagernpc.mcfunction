...
execute if score @s rng1 matches 23 at @s as @p[tag=SEAPT] at @s run tellraw @a[distance=0..50] [{text:"邪教徒弩手：",color:"green",bold:1b},{text:"\n“之前的药品真的十分感谢。你救了我们不少信徒的命。”",color:"white",bold: false}]

execute if score @s rng1 matches 43 at @s run playsound entity.pillager.ambient neutral @a ~ ~ ~ 1 1.15
execute if score @s rng1 matches 43 at @s as @p[tag=SEAPT] at @s run tellraw @a[distance=0..50] [{text:"邪教徒弩手：",color:"green",bold:1b},{text:"\n“我刚刚从监控认出是你，第一时间告诉了其他人。希望你不会怪他们出手攻击你。”",color:"white",bold: false}]

execute if score @s rng1 matches 73 at @s run playsound entity.pillager.ambient neutral @a ~ ~ ~ 1 1.15
execute if score @s rng1 matches 73 at @s as @p[tag=SEAPT] at @s run tellraw @a[distance=0..50] [{text:"邪教徒弩手：",color:"green",bold:1b},{text:"\n“你的恩情我终生不会忘记，无论你最后想要做什么，都祝你成功！”",color:"white",bold: false}]
execute if score @s rng1 matches 73 at @s run tellrawraw @a[tag=SEAPT,advancements={skyblock:sea/doc/g27=false}] [{text:"永久奖励：",color:"green",bold:1b},{text:"喷火器的充能速度稍许加快",color: "white"}]
