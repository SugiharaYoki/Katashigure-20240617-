...
execute if score SEA_ch5_event_illager_count rng1 matches 30.. unless entity @n[tag=SEAmob,type=breeze] run tellraw @a[distance=0..200] [{text:"汉姆：",color:"green",bold:1b},{text:"\n“兄弟们，快上，帮他们解决那个尸变体！”",color:"white",bold: false}]
execute if score SEA_ch5_event_illager_count rng1 matches 30.. if entity @n[tag=SEAmob,type=breeze] run tellraw @a[distance=0..200] [{text:"汉姆：",color:"green",bold:1b},{text:"\n“愣着快什么？他们死了咱们都得死！快去帮忙！”",color:"white",bold: false}]
