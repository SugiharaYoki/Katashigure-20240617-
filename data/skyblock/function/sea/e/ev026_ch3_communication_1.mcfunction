...
execute as @s[tag=sc,scores={sea_4temp2=103}] run scoreboard players add @n[tag=sc] sea_4temp2 3
...
execute as @s[tag=sc,scores={sea_4temp2=172}] positioned 90133 137 108 as @p[tag=SEAPT] \
 run tellraw @a[distance=0..50] {"text":"玛瑞莲：“这植物唯一的弱点是高频声波。\n邪教徒显然意识到了这一点，他们占领了整座通讯中心，关停了通讯阵列。”","color":"dark_purple"}
...
execute as @s[tag=sc,scores={sea_4temp2=203}] run scoreboard players add @n[tag=sc] sea_4temp2 3
execute as @s[tag=sc,scores={sea_4temp2=232}] positioned 90133 137 108 as @p[tag=SEAPT] \
 run tellraw @a[distance=0..50] {"text":"玛瑞莲：“没错……您得想办法到那里操控卫星天线。\n办公区域的天线只能用来即时通话，无法进行广播，因此我这里无能为力。”","color":"dark_purple"}
...
execute as @s[tag=sc,scores={sea_4temp2=270}] run scoreboard players add @n[tag=sc] sea_4temp2 3
