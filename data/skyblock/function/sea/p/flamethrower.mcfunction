...
execute as @n[type=interaction,tag=flamethrowerdetect] on attacker if items entity @s weapon.mainhand shears if entity @s[level=1..] anchored eyes unless block ~ ~ ~ water run function skyblock:sea/p/flamethrower_detect
execute as @n[type=interaction,tag=flamethrowerdetect] on target if items entity @s weapon.mainhand shears if items entity @s container.* minecraft:firework_star anchored eyes unless block ~ ~ ~ water run function skyblock:sea/p/flamethrower_shotgun
