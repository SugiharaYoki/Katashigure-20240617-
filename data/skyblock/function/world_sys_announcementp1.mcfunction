...
execute as @a[x=102,y=5,z=-242,dx=58,dy=300,dz=58] at @s run item replace entity @s container.2 with blaze_powder[minecraft:custom_name='{"text":"阻绝粉","italic":false,"color":"green","bold":true}',lore=['{"text":"掷出该道具，清除附近2格内的荧光记号。","italic":false,"color": "green"}']]
...
execute as @n[x=102,y=5,z=-242,dx=58,dy=300,dz=58,type=item,scores={rng2=3}] at @s if entity @s[nbt={Item:{id:"minecraft:blaze_powder"}}] run particle white_smoke ~ ~ ~ 0.2 0.3 0.2 0.03 10
