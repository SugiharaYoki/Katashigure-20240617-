...
execute if score stageSeconds Azr_system matches 1 run tellraw @a[tag=DebugMode,tag=azrPlayer] [{"text":"[DEBUG MODE MESSAGE] You are playing \"Stage 1\", with playerCount = "},{"score":{"objective":"Azr_system","name":"playerCount"}},{"text":" Maximum Seconds = 40"}]
...
execute if score stageSeconds Azr_system matches 28 positioned -79930.5 38.0 3.5 run function skyblock:azr/effects/basalt_open
...
execute if score stageSeconds Azr_system matches 40.. run tellraw @a[tag=azrPlayer,tag=!hasLifeVitae] {"text":"打开背包用鼠标选定其以开启商店。"}
...
execute if score stageSeconds Azr_system matches 40.. run advancement grant @a[tag=azrPlayer] only skyblock:azr/azr_stage1
