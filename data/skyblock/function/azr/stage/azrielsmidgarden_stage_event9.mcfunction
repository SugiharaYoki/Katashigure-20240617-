...
execute if entity @a[x=-79906,y=40,z=-144,distance=0..2,tag=azrPlayer,tag=!ZaneiTalkA] run function skyblock:tool_rng
...
execute as @a[x=-79906,y=40,z=-144,distance=0..2,tag=azrPlayer,tag=!ZaneiTalkA] at @s if score @e[tag=sc,limit=1] SeGa_NumPartic matches 1 if score @e[tag=sc,limit=1] rng7 matches 6 run tellraw @s {"text":"“吾辈并不清楚汝的死期为何会这么早。但见到了汝在花园的勇猛战绩后，吾辈猜测汝是在生前……不，还是请你认真做自己手头上的事吧。”","color":"white"}
execute as @a[x=-79906,y=40,z=-144,distance=0..2,tag=azrPlayer,tag=!ZaneiTalkA] at @s if score @e[tag=sc,limit=1] SeGa_NumPartic matches 2.. if score @e[tag=sc,limit=1] rng7 matches 6 run tellraw @s {"text":"“吾辈并不清楚汝等的死期为何会这么早。但见到了汝等在花园的勇猛战绩后，吾辈猜测汝等是在生前……不，还是请各位认真做自己手头上的事吧。”","color":"white"}
...
execute as @a[x=-79906,y=40,z=-144,distance=0..2,tag=azrPlayer,tag=!ZaneiTalkA] at @s run function skyblock:azr/ingame_azrielsmidgarden_shop_reader_arrow
