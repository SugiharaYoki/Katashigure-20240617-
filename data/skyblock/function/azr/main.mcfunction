...
#投掷道具
execute as @e[type=item,nbt={OnGround:1b}] if items entity @s container.* shulker_shell[custom_data~{arrow_box:1b}] at @s on origin as @n[distance=..0.01,type=item] run function skyblock:azr/system/player/weapons/arrow_box/summon
...
# 部分关卡检测玩家位置在这里处理
    ...
    # event13
    execute if score stage Azr_system matches 52..54 run function skyblock:azr/assets/events/stage/stage_event14
    execute if score stage Azr_system matches 56..58 run function skyblock:azr/assets/events/stage/stage_event14
    # event18
