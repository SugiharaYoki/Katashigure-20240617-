...
execute positioned -79385 23 -394 as @n[tag=AzrielMob_StoryIntroduction_NPC10,distance=..100,type=piglin_brute,scores={rng1=16}] at @s if entity @p[tag=azrPlayer,distance=..4.4] run tellraw @a[tag=azrShowDialog] [{"text":"守卫：","color":"yellow"},{"text":"\n“看你这么紧张，给你一些物资。你会从那群神界军身上获得更多的。”","color":"white"}]
...
execute if score Azr_Story_Introduction rng1 matches 1024 at @p[tag=azrPlayer] run tellraw @a[tag=azrShowDialog] [{"text":"温柔的声音：","color":"dark_purple"},{"text":"\n“因为，你终将毁灭神庭，也终将拯救它。”","color":"white"}]
...
execute if score Azr_Story_Introduction rng1 matches 1025 positioned -79977 8 61 run summon minecraft:nether_star ~ ~ ~ {Invisible:1b,Fixed:0b,Invulnerable:0b,Facing:1b,Item:{id:"minecraft:nether_star",components:{custom_name:'{"text":"下界命星","color":"yellow","italic":false}',custom_data:{revival_star:true},custom_model_data:{floats:[100001.0]}}}}
