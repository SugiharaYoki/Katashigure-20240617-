...
execute if score stage_main_thread AzrTimerStack matches 27 as @p[tag=azrPlayer] at @s unless block -79929 39 125 air run title @a[tag=azrShowDialog] subtitle [{text:"掌司刑决而虚怀若谷",color:"dark_red",bold:1b}]
execute if score stage_main_thread AzrTimerStack matches 27 as @p[tag=azrPlayer] at @s if block -79929 39 125 air run title @a[tag=azrShowDialog] subtitle [{text:"创生万灵而恃才傲物",color:"dark_red",bold:1b}]
...
#execute if score stage_main_thread AzrTimerStack matches 204 run tellraw @a[scores={Azr_skillPoints=..5}] [{text:"永久升级：",color:"light_purple",bold:1b},{text:"开始新游戏时，绿宝石数量 +5",color:"white",bold: false}]
