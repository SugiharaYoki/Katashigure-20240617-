...
execute if score @s rng1 matches 356 as @a[distance=..200,tag=azrShowDialog] at @s run tellraw @s [{text:"诡谲之低语：",color:"red",bold:1b},{text:"\n“人类的王欢，尔岂非从火中抽出来的一根柴吗？”",color:"white",bold: false}]
execute if score @s rng1 matches 365 as @a[distance=..200,tag=azrShowDialog] at @s run tellraw @s [{text:"诡谲之低语：",color:"red",bold:1b},{text:"\n“照亮着黑暗却燃烧着自己的未来，希望不是从这之中诞生的。”",color:"white",bold: false}]
