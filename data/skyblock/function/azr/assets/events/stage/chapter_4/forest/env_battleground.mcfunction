...
execute store result score @s rng4 run random value 1..480

execute if score @s rng4 matches 2..9 positioned -79785 40 -511 as @n[tag=AzrielTrader_pig_gavroche,distance=..100] at @s run playsound minecraft:entity.piglin.celebrate hostile @a[tag=azrShowDialog] ~ ~ ~ 1 1.1
...
execute if score @s rng4 matches 8 positioned -79785 40 -511 as @n[tag=AzrielTrader_pig_gavroche,distance=..100] at @s run tellraw @a[tag=azrShowDialog,distance=..30] [{text:"猪灵孩童 伽弗洛什：",color:"green",bold:1b},{bold: false,text:"\n“你们绝对不敢相信！前线传来的新闻说，路西法居然能完美吸收生灵魔力！！”",color:"white"}]
execute if score @s rng4 matches 9 positioned -79785 40 -511 as @n[tag=AzrielTrader_pig_gavroche,distance=..100] at @s run tellraw @a[tag=azrShowDialog,distance=..30] [{text:"猪灵孩童 伽弗洛什：",color:"green",bold:1b},{bold: false,text:"\n“路西法曾经是神庭的天使长？！你们决不能错过这种重磅新闻！！”",color:"white"}]
