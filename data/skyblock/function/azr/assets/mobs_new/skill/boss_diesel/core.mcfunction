...
#ACTION
    ...
    execute as @n[tag=AzrielBossDiesel] at @s if score @s[scores={Health=180..250}] rng8 matches 1 unless entity @s[scores={rng2=1..}] store result score @s rng2 run random value 1..6
    execute as @n[tag=AzrielBossDiesel] at @s if score @s[scores={Health=..179}] rng8 matches 1 unless entity @s[scores={rng2=1..}] store result score @s rng2 run random value 1..8
    execute as @n[tag=AzrielBossDiesel] at @s if score @s[scores={rng2=1..3}] rng8 matches 1.. run say hi

    execute as @n[tag=AzrielBossDiesel] at @s if entity @s[scores={Health=251..}] run scoreboard players set @s rng8 -20
    execute as @n[tag=AzrielBossDiesel] at @s if score @s rng8 matches 999.. run scoreboard players set @s rng8 -20
...
execute if score @s AzrEntityTimer matches ..1999 unless entity @a[tag=azrPlayer,x=-79937,dx=11,y=26,dy=8,z=-17,dz=25] run bossbar remove azr:boss_hp_bar_diesel
execute if score @s AzrEntityTimer matches ..1999 unless entity @a[tag=azrPlayer,x=-79937,dx=11,y=26,dy=8,z=-17,dz=25] run tp @n[tag=AzrielBossDiesel] ~ ~-200 ~
execute if score @s AzrEntityTimer matches ..1999 unless entity @a[tag=azrPlayer,x=-79937,dx=11,y=26,dy=8,z=-17,dz=25] run kill @n[tag=AzrielBossDiesel]
execute if score @s AzrEntityTimer matches ..1999 unless entity @a[tag=azrPlayer,x=-79937,dx=11,y=26,dy=8,z=-17,dz=25] run function skyblock:azr/lifecycle/endgame/reset_map_boss_sub_diesel
