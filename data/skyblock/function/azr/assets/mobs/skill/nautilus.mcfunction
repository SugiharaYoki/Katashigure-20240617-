...
execute at @s rotated as @s positioned ^ ^ ^1 as @n[type=item,distance=..1.5] at @s if block ~ ~ ~ #skyblock:water run kill @s

execute at @s[tag=!AzrMob_nautilus_havetarget] facing entity @n[type=item,distance=..9] feet positioned ^ ^ ^0.2 if block ~ ~ ~ #skyblock:water run tp @s ^ ^ ^0.15
