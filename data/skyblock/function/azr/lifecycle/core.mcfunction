...
execute if score #is_stopped:stage_main_thread AzrTimerStack matches 1.. if items entity @a[tag=azrPlayer] weapon.* *[custom_data~{azr_amulet_stopwatch:1b}] run title @a[tag=azrShowDialog] actionbar {"text": "- 计 时 暂 停 -","color": "#debc66","bold": true}
