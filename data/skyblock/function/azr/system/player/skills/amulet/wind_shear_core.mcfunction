...
execute if entity @s[tag=WindShearSuccess] at @s positioned ^ ^1.45 ^3.5 as @e[tag=!AzrielMob_amulet_wind_shear_target,tag=AzrielMob,distance=..1.9] at @s unless score @s AzrSariel_Amulet_WindShear_timer matches 1.. run scoreboard players remove @p[tag=WindShearSuccess] AZR_chainKill_chargeup 3
execute if entity @s[tag=WindShearSuccess] at @s positioned ^ ^1.45 ^3.5 as @e[tag=!AzrielMob_amulet_wind_shear_target,tag=AzrielMob,distance=..1.9] at @s run scoreboard players set @s AzrSariel_Amulet_WindShear_timer 10
