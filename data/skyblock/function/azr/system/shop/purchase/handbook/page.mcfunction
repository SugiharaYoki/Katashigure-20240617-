...
$execute unless data storage azr:handbook_vitae doc.$(eid).skeleton_melee run tellraw @s [{"text":"★？","color":"#6f8095"}]
$execute if data storage azr:handbook_vitae doc.$(eid).skeleton_melee run tellraw @s [{"text":"★骸骨","color":"#4495ff","hoverEvent":{"action":"show_text","contents":[{"text":"类型：亡魂\n","color":"#9c55ff","bold": true},{"text":"攻击方式：近战\n","color":"#ffa463","bold": true},{"text":"羸弱且行动思维单一的怪物。\n但在生命值较低时会陷入假死状态，并快速恢复自身生命值。\n需要注意的是，它的生命值上限要远高于初始生命值，所以一定不能让它获得治疗。","color":"white","bold": false}]}}]
