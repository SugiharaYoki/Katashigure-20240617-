...
execute if items entity @s[predicate=skyblock:sneak] weapon.offhand *[custom_data~{skill_arrow:1b}] run function skyblock:azr/system_sub/skills/arrow/trigger
execute unless items entity @s[predicate=skyblock:sneak] weapon.mainhand * run item replace entity @s weapon.mainhand with music_disc_13[!jukebox_playable,custom_data={skill_arrow:1b,skill_arrow_temp:1b},custom_name='{"text":"「箭」","color": "light_purple","italic": false,"bold": true}']
