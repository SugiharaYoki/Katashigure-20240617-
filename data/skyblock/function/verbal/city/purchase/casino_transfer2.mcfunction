...
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[scores={Perm_PersonFSB=..499}] run tellraw @s {"text":"转账失败！余额不足500浮世币！","color":"red"}
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[scores={Perm_PersonFSB=500..}] run playsound minecraft:item.armor.equip_iron neutral @s ~ ~ ~ 100
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[scores={Perm_PersonFSB=500..}] run playsound minecraft:item.armor.equip_iron neutral @p[distance=0.01..10,gamemode=!spectator] ~ ~ ~ 100
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[scores={Perm_PersonFSB=500..}] run tellraw @s [{"text":"转账500FSB成功！转账对象： ","color":"green"},{"selector":"@p[distance=0.01..10,gamemode=!spectator]","color":"blue"}]
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[scores={Perm_PersonFSB=500..}] run tag @s add purchasesuccess
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[tag=purchasesuccess] run scoreboard players remove @s Perm_PersonFSB 500
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[tag=purchasesuccess] run scoreboard players add @p[distance=0.01..10,gamemode=!spectator] Perm_PersonFSB 500
execute if entity @a[distance=0.01..10,gamemode=!spectator] if entity @s[tag=purchasesuccess] run tellraw @p[distance=0.01..10,gamemode=!spectator] [{"text":"收到了500FSB的转账！来自： ","color":"green"},{"selector":"@s","color":"blue"}]
