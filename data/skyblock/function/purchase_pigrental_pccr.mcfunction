...
execute if entity @n[tag=sc,scores={PigCarCount=41..,rng3=2}] run tellraw @s {"text":"中枢猪车公司·接待员：","bold":true,"color":"aqua"}
execute if entity @n[tag=sc,scores={PigCarCount=41..,rng3=2}] run tellraw @s {"text":"“这里只有40只豪车——目前全部都被租走了。真的很抱歉。”","bold":false,"color":"white"}
execute if entity @n[tag=sc,scores={PigCarCount=41..,rng3=3}] run tellraw @s {"text":"中枢猪车公司·接待员：","bold":true,"color":"aqua"}
execute if entity @n[tag=sc,scores={PigCarCount=41..,rng3=3}] run tellraw @s {"text":"“很遗憾……我们已经没有多余的豪车了。请过段时间再来看看吧！”","bold":false,"color":"white"}
...
execute if entity @n[tag=sc,scores={PigCarCount=..40}] as @s at @s[tag=purchasesuccess] run tellraw @s {"text":"中枢猪车公司·接待员：","bold":true,"color":"aqua"}
...
execute if entity @n[tag=sc,scores={PigCarCount=..40}] as @s at @s[tag=purchasesuccess] run summon pig -9 54 -28 {Tags:[PigCar],Saddle:1,Rotation:[-90f,0f],Attributes:[{Name:generic.max_health,Base:80.0},{Name:generic.movement_speed,Base:0.5}],Health:80.0f,CustomName:"\"豪车\""}
