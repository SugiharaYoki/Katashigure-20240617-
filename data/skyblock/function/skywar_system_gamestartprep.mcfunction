...
execute if score @n[tag=sc] SSgspT matches 2 run scoreboard objectives add If_Bless11 dummy
...
execute if score @n[tag=sc] SSgspT matches 2 run scoreboard objectives add If_Bless30 custom:damage_blocked_by_shield
...
execute if score @n[tag=sc] SSgspT matches 3 as @a[tag=!NoSkyWar] at @s run scoreboard players set @s If_Bless11 0
...
execute if score @n[tag=sc] SSgspT matches 3 as @a[tag=!NoSkyWar] at @s run scoreboard players set @s If_Bless30 0
...
execute if score @n[tag=sc] SSgspT matches 1 if block -131 58 -133 diamond_block run tellraw @a[tag=!Gaming] {"text":"◇ 伊什塔尔的赐福 已开启","color":"aqua","bold": true}
