...
function skyblock:pvp/skywar/bless/setting/bless_macro {job:'"建筑师"', title:'"錾艺「资源集成」"', description:'"特殊模板 不置于副手时，获得某些方块后将自动转换为另一种方块\\\\n土壤类将会成为普通的土、石类将会成为普通的石头\\\\n受影响方块：完整的花岗岩、安山岩、闪长岩、圆石、苔圆石、黑石、深板岩、凝灰岩、方解石、草方块、灰化土、砂土、缠根土、菌丝、绯红岩、诡异岩、红沙等\\\\n\\\\n连携效果：若存在友方「理性混淆」持有者，\\\\n受影响的石类方块将转变为风弹并给予友方的全部「理性混淆」持有者"', id: 033, idsh: 33}
execute at @n[tag=id_data_reading,type=marker] if items block ~ ~ ~ container.10 green_wool run function skyblock:pvp/skywar/bless/setting/bless_macro {job:'"圣女"', title:'"救赎「顷刻天国」"', description:'"顷刻天国核心×2 范围内敌我飘升并发光6秒\\\\n留在背包内时不会陷入中毒状态"', id: 010, idsh: 10}
execute at @n[tag=id_data_reading,type=marker] unless items block ~ ~ ~ container.10 green_wool run tellraw @s [{"text":"    圣女 救赎「顷刻天国」","color":"gold","hoverEvent":{"action":"show_text","contents":{"text":"顷刻天国核心×2 范围内敌我飘升并发光6秒\n留在背包内时不会陷入中毒状态","color":"white"}}},{"text":" 购买 (50 SHD) ","color":"gray","clickEvent":{"action":"run_command","value":"/trigger MultiMenu set 139010"}}]
