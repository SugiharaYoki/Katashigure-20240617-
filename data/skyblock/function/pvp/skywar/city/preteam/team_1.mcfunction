...
execute positioned -135 57 -98 as @a[distance=0..5] at @s if entity @s[tag=!PVPTeamed] if block ~ ~-1 ~ lime_concrete run tellraw @a[tag=!Gaming,tag=!NoSkyWar] [{"text": "组队预选", "bold": true, "color": "yellow"},{"text": " 新的玩家加入了A队", "bold": false, "color": "white"}]
