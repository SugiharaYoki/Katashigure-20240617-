...
execute if score @s 4ASCEND_HP_Trigger matches ..1 run tellraw @s {"text":"输入的值不符合范围！(min=2,max=5)","color":"red"}
execute if score @s 4ASCEND_HP_Trigger matches 6.. run tellraw @s {"text":"输入的值不符合范围！(min=2,max=5)","color":"red"}
