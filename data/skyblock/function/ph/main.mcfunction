execute as @a[tag=!Gaming] run function skyblock:ph/main/all_player
function skyblock:ph/tool/table_manager

#invitation management
execute if score hasInvitation 4ASCEND_system matches 1 unless entity @a[tag=4ASCENDInvite] run function skyblock:ph/invite/expire
execute if score hasInvitation 4ASCEND_system matches 0 run function skyblock:ph/invite/clear

#4ASCEND runtime
data modify storage ph runtime.table set value 0
execute unless data storage ph {table_manager:[{id:0,available:1b}]} run function skyblock:ph/runtime/core
data modify storage ph runtime.table set value 1
execute unless data storage ph {table_manager:[{id:1,available:1b}]} run function skyblock:ph/runtime/core
data modify storage ph runtime.table set value 2
execute unless data storage ph {table_manager:[{id:2,available:1b}]} run function skyblock:ph/runtime/core
data modify storage ph runtime.table set value 3
execute unless data storage ph {table_manager:[{id:3,available:1b}]} run function skyblock:ph/runtime/core
data modify storage ph runtime.table set value 4
execute unless data storage ph {table_manager:[{id:4,available:1b}]} run function skyblock:ph/runtime/core