...
tellraw @s [{"text":"      Minigame - 亚兹列尔的中庭花园 最高记录： ","color":"dark_purple"},{"score":{"name":"@s","objective":"SeGa_StandLastBH"}}]
tellraw @s {"text":" "}
tellraw @s [{"text":"   ","color":"white"},{"text":"查看职业数据","underlined":true,"color":"yellow","clickEvent":{"action":"run_command","value":"/trigger MultiMenu set 29"},"hoverEvent":{"action":"show_text","contents":{"text":"查看自己的PVP职业统计数据","color":"white"}}}]
tellraw @s {"text":" "}
